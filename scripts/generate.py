...
BITS = int(sys.argv[4]) if len(sys.argv) > 4 else 0   # 0 = bf16, else int4/int8
OUT = sys.argv[5] if len(sys.argv) > 5 else "gen_smoke.png"

qbits = BITS or None
print(f"[gen] loading pipeline (DiT {'int'+str(BITS) if BITS else 'bf16'}) ...")
t0 = time.time()
pipe = LensPipeline.from_pretrained(ROOT / "weights" / "Lens", dit_dtype=mx.bfloat16, quantize_bits=qbits)
